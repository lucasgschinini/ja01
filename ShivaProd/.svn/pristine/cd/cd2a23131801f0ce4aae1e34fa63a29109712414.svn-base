CREATE OR REPLACE VIEW SHV_SOP_BANDEJA_DE_ENTRADA (ID_TAREA, TIPO_TAREA, ESTADO_TAREA, FECHA_CREACION, USUARIO_CREACION, FECHA_EJECUCION, USUARIO_EJECUCION, INFORMACION_ADICIONAL, GESTIONA_SOBRE, PERFIL_ASIGNACION, USUARIO_ASIGNACION, ID_WORKFLOW_ASOCIADO, NUMERO_CLIENTE, RAZON_SOCIAL, IMPORTE, ESTADO_WORKFLOW_ASOCIADO, ID_REGISTRO_AVC, ID_VALOR_POR_REVERSION, ID_VALOR, ID_TALONARIO, ID_OPERACION_MASIVA, ID_COBRO, ID_OPERACION, ID_DESCOBRO, MONEDA_IMPORTE, ID_EMPRESA, DESCRIPCION_EMPRESA, ID_SEGMENTO, DESCRIPCION_SEGMENTO, NUMERO_BOLETA, DESCRIPCION_BANCO, NUMERO_CHEQUE, FECHA_VENCIMIENTO, CODIGO_INTERDEPOSITO, REFERENCIA, TIPO, NRO_CONSTANCIA, CUIT, PROVINCIA, COD_ORGANISMO, RANGO)
AS
  (
  -- FACE 3, Dolares MONEDA_IMPORTE, u578936, Max
  -- SPRINT 8, operaciones masivas
  -- SPRINT 7, u573005, fabio.giaquinta.ruiz, merge del 08/03/2016
  --------------------------------------------------------------------------------
  --------------------------------------------------------------------------------
  --------------------------------------------------------------------------------
  ...
  SELECT
    ...
    NULL AS ID_DESCOBRO,
	-- COBROS / DESCOBRO
	SWTC.MONEDA_IMPORTE AS MONEDA_IMPORTE,
    -- VALOR / TALONARIO / OPERACION MASIVA / COBRO
    SPE.ID_EMPRESA  AS ID_EMPRESA,
    SPE.DESCRIPCION AS DESCRIPCION_EMPRESA,
    SPS.ID_SEGMENTO AS ID_SEGMENTO,
  ...
  SELECT
	...
	-- COBROS / DESCOBRO
	SWTD.MONEDA_IMPORTE AS MONEDA_IMPORTE,
    -- VALOR / TALONARIO / OPERACION MASIVA / COBRO
    SPE.ID_EMPRESA  AS ID_EMPRESA,
    SPE.DESCRIPCION AS DESCRIPCION_EMPRESA,
    SPS.ID_SEGMENTO AS ID_SEGMENTO,
  ...
  SELECT
    ...
    SWT.GESTIONA_SOBRE,
    CASE
      WHEN upper(swt.perfil_asignacion) IN ('ANALISTAOPERACIONMASIVA', 'SUPERVISOROPERACIONMASIVA')
      THEN (SWT.PERFIL_ASIGNACION
        ||'_'
        ||SPE.ID_EMPRESA
        ||'_'
    ...
    NULL AS ID_TALONARIO,
    -- OPERACION MASIVA
    smom.Id_Operacion_Masiva AS ID_OPERACION_MASIVA,
    -- COBROS
    NULL AS ID_COBRO,
    NULL AS ID_OPERACION,
    -- DESCOBROS
    ...
    NULL             AS FECHA_VENCIMIENTO,
    NULL             AS CODIGO_INTERDEPOSITO,
    swtom.REFERENCIA AS REFERENCIA,
    NULL             AS TIPO,
    NULL             AS NRO_CONSTANCIA,
    NULL             AS CUIT,
    NULL             AS PROVINCIA,
  ...
  FROM
    ...
    SHV_WF_TAREA SWT,
    SHV_WF_WORKFLOW_ESTADO SWFE,
    -- FROM DE OPERACIONES MASIVAS
    shv_wf_tarea_operacion_masiva swtom,
    shv_mas_oper_masiva smom,
    SHV_WF_WORKFLOW_ESTADO SWFEENTIDAD,
    SHV_PARAM_EMPRESA SPE,
    SHV_PARAM_SEGMENTO SPS
  WHERE
    ...
    SWT.ID_WORKFLOW = SWFE.ID_WORKFLOW
  AND SWT.ESTADO    = 'PENDIENTE'
    -- WHERE DE OPERACIONES MASIVAS
  AND swt.id_tarea             = swtom.id_tarea
  AND smom.id_operacion_masiva = swtom.id_operacion_masiva
  AND smom.id_workflow         = swfeEntidad.id_workflow
  AND smom.id_empresa          = spe.id_empresa (+)
  AND smom.id_segmento         = sps.id_segmento (+)
  );
--Agrego el campo moneda de la operacion a la busqueda
CREATE OR REPLACE VIEW SHV_SOP_COBROS_BUSQUEDA (ID_COBRO, ID_COBRO_PADRE, ID_EMPRESA, ID_SEGMENTO, ID_OPERACION, ID_MOTIVO_COBRO, DESC_MOTIVO_COBRO, CLIENTE, ESTADO, ANALISTA, COPROPIETARIO, ID_ANALISTA_TEAM_COMERCIAL, ID_OPERACION_MASIVA, NOMBRE_ARCHIVO, IMPORTE_TOTAL, FECHA_ALTA, FECHA_DERIVACION, FECHA_APROB_SUPER_COB, FECHA_APROB_REFER_COB, FECHA_IMPUTACION, SUB_ESTADO, FECHA_ULTIMO_ESTADO, TIENE_DUC, MONEDA_IMPORTE)
AS (
  SELECT SCEC.ID_COBRO   AS IDCOBRO,
    ...
    SMOP.ID_OPERACION_MASIVA   AS idOperacionMasiva,
    SMOPA.NOMBRE_ARCHIVO       AS nombreArchivo,
    ...
    CASE
      ...
      ELSE 'SI'
    END AS TIENE_DUC,
  SCEC.MONEDA_OPERACION AS MONEDA_IMPORTE
  ...
  LEFT JOIN SHV_MAS_OPER_MASIVA SMOP
  ON SCEC.ID_OPERACION_MASIVA = SMOP.ID_OPERACION_MASIVA
  LEFT JOIN SHV_MAS_OPER_MASIVA_ARCH SMOPA
  ON SMOP.ID_OPERACION_MASIVA = SMOPA.ID_OPERACION_MASIVA
 );
Exit;