-----------------------------------------------------------------------------------------------------------------------------
-- Actualización de la vista de Subdiario (Defecto 1016, temas varios de reveriones)
-----------------------------------------------------------------------------------------------------------------------------
...
(
  ...
  SELECT WE.FECHA_MODIFICACION AS FECHAPROCESAMIENTO,
    ...
    NULL                                    AS IMPORTE_MONEDA_ORIGEN,
    --
    -- Calculo de fecha valor de la transaccion a la que pertenece el medio de pago
    --
    (select fecha_valor from shv_cob_factura 
     where id_transaccion = tran.id_transaccion
     union 
     select fecha_valor from shv_cob_tratamiento_diferencia
     where id_transaccion = tran.id_transaccion) as fechavalor,
    --
    OP.ID_CAJA                              AS IDCAJA,
    --
    -- Datos para uso interno
