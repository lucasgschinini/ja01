...
AS
  ...
  SELECT
	...
	case 
		when
			(scf.estado_cargo_generado = 'FACTURADO') and ((scfm.cobrador_interes_trasla_no_reg + scfm.cobrador_interes_trasla_regu) <> 0)
		then
			(scfm.cobrador_interes_trasla_no_reg + scfm.cobrador_interes_trasla_regu)
		 else 
			scf.cobrador_intereses_generados			
  ...
  SELECT
    ...
    -- Gestion de intereses
    scf.que_hacer_con_intereses        										AS que_hacer_con_intereses,
    NVL(scfc.cobrador_interes_trasla_regu,scf.cobrador_intereses_generados) AS cobrador_intereses_generados,
    scf.cobrador_intereses_trasladados 										AS cobrador_intereses_trasladados,
    scf.check_trasladar_intereses      										AS check_trasladar_intereses,
...
AS
  ...
  FROM shv_param_tipo_medio_pago sptmp,
    ...
    SELECT scmpdpm.id_medio_pago,
	  ...
	  case 
		when
		  (scmpdpm.cobrador_interes_real_nrg_tras <> null) and (scmpdpm.cobrador_interes_real_rg_tras <> 0)
		then
			scmpdpm.cobrador_interes_real_rg_tras
		 else 
			scmpdpm.cobrador_intereses_generados			
	  end "cobrador_intereses_generados",
...
AS
  ...
  SELECT
    ...
    case 
		when
		  (sctd.cobrador_interes_real_nrg_tras <> null) and (sctd.cobrador_interes_real_nrg_tras <> 0)
		then
			sctd.cobrador_interes_real_nrg_tras
		 else 
			sctd.cobrador_intereses_generados			
	end "cobrador_intereses_generados",
