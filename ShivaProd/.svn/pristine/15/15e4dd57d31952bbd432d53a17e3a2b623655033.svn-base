...
AS
  ...
  FROM shv_param_tipo_medio_pago sptmp,
    ...
    SELECT id_medio_pago,
      ...
      NULL AS IMPORTE_APLIC_FEC_EMIS_MON_ORI,
      NULL AS acuerdo_traslado_cargo_orignal
    FROM shv_cob_med_pag_otras_compen
    UNION
    --
...
AS
  ...
  UNION
  ----------------------------------------------------------------------------
  -- Factura Calipso (Cobro) sin hijos Medio de pago SHIVA SHV_COB_MED_PAG_OTRAS_COMPEN
  ----------------------------------------------------------------------------
  SELECT NULL             AS ciclo,
    NULL                  AS tipo_factura,
    NULL                  AS tipo_pago,
  ...
  FROM shv_cob_cobro cob,
    ...
    shv_cob_factura_calipso fcal,
    shv_cob_med_pago mp,
    SHV_COB_MED_PAG_OTRAS_COMPEN mpShivaD
  WHERE cob.id_workflow            = we.id_workflow
  AND cob.id_operacion             = op.id_operacion
  AND op.id_operacion              = tran.id_operacion
  AND tran.id_transaccion          = fac.id_transaccion
  ...
  FROM shv_cob_cobro cob,
    ...
    shv_cob_factura_calipso fcal,
    shv_cob_med_pago mp,
    SHV_COB_MED_PAG_OTRAS_COMPEN mpShivaC
  WHERE cob.id_workflow            = we.id_workflow
  AND cob.id_operacion             = op.id_operacion
  AND op.id_operacion              = tran.id_operacion
  AND tran.id_transaccion          = fac.id_transaccion
  ...
  UNION
  ------------------------------------------------------------------------------
  -- Facturas Calipso (Cobro) con hijos por diferencia de cambio Medio pago SHIVA SHV_COB_MED_PAG_OTRAS_COMPEN
  ------------------------------------------------------------------------------
  SELECT NULL             AS ciclo,
    NULL                  AS tipo_factura,
    NULL                  AS tipo_pago,
