...
public class ImputacionCobroManualCallable implements Callable<ResultadoImputacionCobro> {
	
	private String idThread;
	private ImputacionCobroRto datosEntrada;
	
	public ImputacionCobroManualCallable(ImputacionCobroRto datosEntrada) {
...
//		datosEntrada.getCobroAProcesar().setTipoImputacion(TipoImputacionEnum.IMPUTACION_MANUAL_COBROS);
//		
//		procesamientoHilosCobrosServicio.insertarActualizarHilo(datosEntrada.getCobroAProcesar());
		...
		try {
			...
			Thread.currentThread().setName(TrazaEnum.IMPUTACION_MANUAL_COBROS.name());
	        ...
	        cobroServicio.imputarCobroManual(datosEntrada);
