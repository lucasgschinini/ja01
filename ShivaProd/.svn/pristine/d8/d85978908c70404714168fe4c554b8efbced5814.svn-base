...
public class FacConsultaAcuerdoxClienteMapeador 
	extends MapeadorJMS {
	...
	public JMS deserializar(String mensajeRecibido, boolean camposSeteables, boolean esSincronico) throws NegocioExcepcion {
		JmsRetorno jmsRetorno = null;
		int longitudCabecera = 10;
		int cantidadRegistros = 1;
		//reseteo los contadores de posiciones
		posInicial = 0;
		posFinal = 0;
		
		if (mensajeRecibido.length() >= longitudCabecera) {
			
			jmsRetorno = getCopiaTecnica(mensajeRecibido, camposSeteables);
			if (!Validaciones.isObjectNull(jmsRetorno)) {
				cantidadRegistros = new Integer(jmsRetorno.getCantRegistrosRetornados());
			} else {
				String mensajeError = "Este mensaje no tiene el encabezado";
		    	throw new NegocioExcepcion(mensajeError);
			}
		}
		
		int longitudRegistrosTotal = 
				(getDefaultFormatoMensajeJMS().getLontitudFijaMensajeDeserializable()-longitudCabecera) * cantidadRegistros;
		
		boolean esMensajeDeError = false;
		if ((longitudCabecera + longitudRegistrosTotal) == mensajeRecibido.length()) {
		...
		// seteo el retorno que ya esta resuelto
		datosRecibidos.setRetorno(jmsRetorno);
		//saco el retorno de los campos de entrada para no recorrerlo en el for
		camposEntrada.remove(0);
		    	...
		    	if (!esMensajeDeError) {    			    			
			    	...
			    	if (campo.getTipoDato().equals(TipoDatoEnum.NUMERICO)) {
			    		if ("INVC-ARGT-ID-DSP".equalsIgnoreCase(nombreCampo)) {
			    			try {
			    				posFinal = posInicial + longitud;
			    		    	Long acuerdo = Long.valueOf(mensajeRecibido.substring(posInicial, posFinal));
	...
	/**
	 * Retorna la cabecera del mensaje
	 * @param mensajeRecibido
	 * @param camposSeteables
	 * @return
	 * @throws NegocioExcepcion
	 */
	private JmsRetorno getCopiaTecnica(String msg, boolean camposSeteables) throws NegocioExcepcion {
