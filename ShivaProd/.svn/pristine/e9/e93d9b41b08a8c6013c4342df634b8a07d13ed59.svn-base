(function() {
	...
	var getType = (function() {
		var toString = Object.prototype.toString, toStrings = {}, nodeTypes = {
			1 : "element",
			3 : "textnode",
			9 : "document",
			11 : "fragment"
		}, types = "Arguments Array Boolean Date Document Element Error Fragment Function NodeList Null Number Object RegExp String TextNode Undefined Window"
				.split(" ");
		for (var i = types.length; i--;) {
			var type = types[i], constructor = window[type];
			if (constructor) {
				try {
					toStrings[toString.call(new constructor)] = type
							.toLowerCase();
				} catch (e) {
				}
			}
		...
		return function(item) {
			return item == null
					&& (item === undefined ? _undef : "null")
					|| item.nodeType
					&& nodeTypes[item.nodeType]
					|| typeof item.length == "number"
					&& (item.callee && _arguments || item.alert && "window" || item.item
							&& "nodelist") || toStrings[toString.call(item)];
		};
	})();
	...
	var weaveOne = function(source, method, advice) {
		...
		var aspect;
		if (advice.type == _after || advice.type == _afterThrow
				|| advice.type == _afterFinally) {
			aspect = function() {
				...
				if (advice.type == _after) {
					if (exceptionThrown == null) {
						returnValue = advice.value.apply(this, [ returnValue,
								method ]);
					} else {
					...
					if (advice.type == _afterThrow && exceptionThrown != null) {
						returnValue = advice.value.apply(this, [
								exceptionThrown, method ])
					} else {
						if (advice.type == _afterFinally) {
							returnValue = advice.value.apply(this, [
									returnValue, exceptionThrown, method ]);
						}
					}
				}
				return returnValue;
					...
					if (advice.type == _around) {
						aspect = function() {
							...
							return advice.value.apply(invocation.object, [ {
								...
								method : method,
								proceed : function() {
									return old.apply(invocation.object,
											invocation.args)
								}
							} ]);
						};
	...
	var weave = function(pointcut, advice) {
		try {
			var source = typeof (pointcut.target.prototype) != _undef ? pointcut.target.prototype
					: pointcut.target;
			var advices = [];
			if (advice.type != _intro && typeof (source[pointcut.method]) == _undef) {
				...
				for ( var i in methods) {
					advices[advices.length] = weaveOne(methods[i].source,
							methods[i].method, methods[i].advice);
				}
			} else {
...
$(document).ready(function() {
	...
	if (!jQuery.browser.webkit && !jQuery.browser.safari) {
	    ...
	    style.value = "overflow: auto; color: red; position: fixed; left:0; bottom:0; background-color: black; height:250px; width:350px; filter: alpha(opacity=80);";
	 ...
	 * AOP
	 **********************************************************/
	$.aop.around( {target: window, method: 'guardarCobro'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
		$.tracear('['+invocation.method+'] ejecutado en ' + dif + ' s.');
	});
	$.aop.around( {target: window, method: 'actualizarInfoTabla'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
	...
	$.aop.around( {target: window, method: 'validarEstadoDeimos'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
	...
	$.aop.around( {target: window, method: 'prepararNuevaListaSeleccionados'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		var retorno = invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
		$.tracear('['+invocation.method+'] ejecutado en ' + dif + ' s.');
		return retorno;
	});
	$.aop.around( {target: aparienciaButton, method: 'ocultarButtonPlus'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
	...
	$.aop.around( {target: aparienciaButton, method: 'descubrirButtonPlus'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
	...
	$.aop.around( {target: aparienciaButton, method: 'ocultarbuttonPlusById'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
	...
	$.aop.around( {target: aparienciaButton, method: 'descubrirButtonPlusCliente'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
	...
	$.aop.around( {target: aparienciaButton, method: 'descubrirButtonPlusClienteTodos'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
	...
	$.aop.around( {target: aparienciaButton, method: 'ocultarButtonClientesSeleccionados'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
	...
	$.aop.around( {target: aparienciaButton, method: 'descubrirButtonPlusDocumentosTodos'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
	...
	$.aop.around( {target: aparienciaButton, method: 'ocultarButtonCreditoDebitoSeleccionados'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
	...
	$.aop.around( {target: aparienciaButton, method: 'descubrirButtonPlusDocumento'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
	...
	$.aop.around( {target: aparienciaButton, method: 'ocultarButtonPlusDocumento'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
	...
	$.aop.around( {target: aparienciaButton, method: 'ocultarButtonCreditosDebitosSeleccionados'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
	...
	$.aop.around( {target: aparienciaButton, method: 'ocultarButtonDocumentosSeleccionados'}, function(invocation) {
		var start = new Date().getTime()/1000.0;
		invocation.proceed();
		var fin = new Date().getTime()/1000.0;
		var dif= fin-start;
