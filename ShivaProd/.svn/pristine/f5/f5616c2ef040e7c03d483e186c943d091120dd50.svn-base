...
public class NumerosALetras {
	...
	private final String[] CENTAVOS = { "diez ", "once ", "doce ", "trece ",
			"catorce ", "quince ", "dieciseis ", "diecisiete ", "dieciocho ",
			"diecinueve", "veinte ", "treinta ", "cuarenta ", "cincuenta ",
			"sesenta ", "setenta ", "ochenta ", "noventa " };
	...
	public String Convertir(String numero, boolean mayusculas) {
		...
		if (Pattern.matches("\\d{1,12},\\d{1,2}", numero)) {
			...
			if(Num[1].equals("01")){
				parte_decimal = getCentavos(Num[1]) + "centavo";
			} else {
				parte_decimal = getCentavos(Num[1]) + "centavos";
			}
	...
	private String getCentavos(String num) {// 99
		int n = Integer.parseInt(num);
		if (n == 0) {
			return "cero ";
		} else if (n < 10) {// para casos como -> 01 - 09
			return getUnidades(num);
		} else if (n > 19) {// para 20...99
			String u = getUnidades(num);
			if (u.equals("")) { // para 20,30,40,50,60,70,80,90
				return CENTAVOS[Integer.parseInt(num.substring(0, 1)) + 8];
			} else {
				return CENTAVOS[Integer.parseInt(num.substring(0, 1)) + 8]
						+ "y " + u;
			}
		} else {// numeros entre 11 y 19
			return CENTAVOS[n - 10];
		}
	}
