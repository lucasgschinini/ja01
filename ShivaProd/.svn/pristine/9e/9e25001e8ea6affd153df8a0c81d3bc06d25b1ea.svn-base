...
NOMBRE_PROCESO=batch-shiva-simulacionDescobros
...
then
	...
	java -cp ".:${CLASSPATH}" -Djava.io.tmpdir=${TEMP} \
		...
		-Dentorno=${ENTORNO} -Dconfiguracion=${SHIVA_CONFIG} -DnombreLog=${NOMBRE_PROCESO} \
   			ar.com.telecom.shiva.batch.SimulacionDescobrosBatchRunner >>$LOG_ERROR >>$LOG_ARCHIVO 2>>$LOG_ERROR
else
	# Solo para produccion
	java -cp ".:${CLASSPATH}" -Dentorno=${ENTORNO} \
		-Xms1303m -Xmx1303m -XX:MaxPermSize=256m \
