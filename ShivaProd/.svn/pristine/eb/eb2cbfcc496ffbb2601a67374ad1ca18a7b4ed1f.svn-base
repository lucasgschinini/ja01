...
Begin
  ...
  execute immediate 'grant select, insert, update, delete on '||'&1'||'.SHV_WF_TAREA_OPERACION_MASIVA'||' to '||'&2';
  dbms_output.put_line('Se ha creado exitosamente el grant al usuario '||'&2'||' para el objeto '||'&1'||'SHV_WF_TAREA_OPERACION_MASIVA');
