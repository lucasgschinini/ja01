...
NOMBRE_PROCESO=batch-shiva-novedadesCajeroPagador
...
then
	...
	java -cp ".:${CLASSPATH}" -Djava.io.tmpdir=${TEMP} \
		-Dentorno=${ENTORNO} -Dconfiguracion=${SHIVA_CONFIG} -DnombreLog=${NOMBRE_PROCESO} \
   			ar.com.telecom.shiva.batch.InformarNovedadesCajeroPagadorBatchRunner >>$LOG_ERROR >>$LOG_ARCHIVO 2>>$LOG_ERROR
else
