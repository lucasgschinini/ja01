...
NOMBRE_PROCESO=batch-shiva-operacionesMasivasLecturaMic
	...
	java -cp ".:${CLASSPATH}" -Djava.io.tmpdir=${TEMP} \
		-Xms1303m -Xmx1303m -XX:MaxPermSize=256m \
	 	-Dentorno=${ENTORNO} -Dconfiguracion=${SHIVA_CONFIG} -DnombreLog=${NOMBRE_PROCESO} \
   		ar.com.telecom.shiva.batch.OperacionesMasivasLecturaArchivoMicBatchRunner >>$LOG_ERROR >>$LOG_ARCHIVO 2>>$LOG_ERROR
else
