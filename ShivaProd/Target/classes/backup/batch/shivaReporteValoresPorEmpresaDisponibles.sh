...
NOMBRE_PROCESO=batch-shiva-reporteValoresPorEmpresaDisponibles
	...
	then
		...
		java -cp ".:${CLASSPATH}" -Djava.io.tmpdir=${TEMP} \
		 	-Dentorno=${ENTORNO} -Dconfiguracion=${SHIVA_CONFIG} -DnombreLog=${NOMBRE_PROCESO} \
	   		ar.com.telecom.shiva.batch.ReporteValoresPorEmpresaDisponiblesBatchRunner $1 >>$LOG_ERROR >>$LOG_ARCHIVO 2>>$LOG_ERROR
	else
