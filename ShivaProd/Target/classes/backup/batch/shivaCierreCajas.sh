...
NOMBRE_PROCESO=batch-shiva-cierreCajas
...
######################### Ejecucion ###########################
echo Fecha de Cierre: $1 >>$LOG_ARCHIVO
...
then
	...
	java -cp ".:${CLASSPATH}" -Djava.io.tmpdir=${TEMP} \
		-Dentorno=${ENTORNO} -Dconfiguracion=${SHIVA_CONFIG} -DnombreLog=${NOMBRE_PROCESO} \
 	  		ar.com.telecom.shiva.batch.CierreCajaBatchRunner $1 >>$LOG_ERROR >>$LOG_ARCHIVO 2>>$LOG_ERROR
else
