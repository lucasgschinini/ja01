...
NOMBRE_PROCESO=batch-shiva-procesarEntradaDetalleAplicacionesManuales
	...
	then
		...
		java -cp ".:${CLASSPATH}" -Djava.io.tmpdir=${TEMP} \
		 	-Dentorno=${ENTORNO} -Dconfiguracion=${SHIVA_CONFIG} -DnombreLog=${NOMBRE_PROCESO} \
	   		ar.com.telecom.shiva.batch.ProcesarEntradaDetalleAplicacionesManualesBatchRunner $1 >>$LOG_ERROR >>$LOG_ARCHIVO 2>>$LOG_ERROR
	else
