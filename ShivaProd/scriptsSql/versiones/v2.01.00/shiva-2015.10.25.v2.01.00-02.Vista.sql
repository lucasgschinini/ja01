--------------------------------------------------------------------------------------------------------------
-- Actualización de la vista de busqueda de cobros
--------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------
-- Actualización de la vista de valores
--------------------------------------------------------------------------------------------------------------
...
AS
  ...
  SELECT
   ...
   (select  RTRIM(
         REGEXP_REPLACE(
           (LISTAGG(LPAD(TRAN.ID_OPERACION,7,'0'), ';') WITHIN GROUP (ORDER BY TRAN.ID_OPERACION)), 
           '([^;]*)(;\1)+($|;)', '\1\3'), ';') as ID_SHIVA
      from SHV_COB_COBRO COBRO, SHV_COB_TRANSACCION TRAN, SHV_COB_MED_PAGO MED, SHV_COB_MED_PAG_SHIVA MEDSHIV
      ...
      and TRAN.ESTADO in('PENDIENTE','APROPIADA','ERROR_CONFIRMACION','ERROR_DESAPROPIACION'
      ,'EN_PROCESO_APROPIACION','EN_PROCESO_CONFIRMACION','EN_PROCESO_DESAPROPIACION','CONFIRMADA')
	  and rownum < 100) AS COBRO_FORMATEADO,
	NULL AS ID_BOLETA,
