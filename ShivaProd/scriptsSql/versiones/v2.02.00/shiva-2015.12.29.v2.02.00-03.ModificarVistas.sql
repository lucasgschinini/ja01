--
-- u573005, fabio.giaquinta.ruiz, se modifica la vista de historial de cobros para mostrar el registro historico del descobro
--
...
SELECT ed_cobro1.id_cobro,
  ...
  SELECT ed_cobro.id_cobro,
    ...
    CASE
      ...
      WHEN (scf.tipo_comprobante) = 'DUC'
      THEN
        (SELECT scfm.ID_REFERENCIA_FACTURA
        FROM shv_cob_factura_mic scfm
  ...
  --levanta el estado descobrado
  UNION
	  select null as id_cobro,
    ED_COBRO.ID_COBRO as id_cobro_padre,
    descobro.id_operacion,
    wf_estado.estado,
    NULL                                                           AS subestado,
    TO_CHAR(wf_estado.fecha_modificacion, 'DD/MM/YYYY HH24:MI:SS') AS fecha_modificacion,
    descobro.id_analista,
    null as observacion,
    null as mensaje_estado,
    null as numero_transaccion,
    null as tipo_comprobante,
  ...
  FROM
    ...
    SELECT swwe.id_workflow,
      ...
      DBMS_LOB.substr(swwe.datos_modificados,4000,1) AS observacion
    FROM shv_wf_workflow_estado swwe
    WHERE swwe.estado = 'DES_DESCOBRADO'
    ) WF_ESTADO
  inner JOIN shv_cob_descobro descobro
  ON wf_estado.id_workflow = descobro.id_workflow
  inner join SHV_COB_ED_COBRO ED_COBRO
  on DESCOBRO.ID_COBRO = ED_COBRO.ID_COBRO 
  );
