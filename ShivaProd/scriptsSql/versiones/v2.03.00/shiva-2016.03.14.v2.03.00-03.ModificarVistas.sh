...
NOMBRE=shiva-2016.03.14.v2.03.00-03.ModificarVistas
...
if [ "$1" != "" ]; then
    ...
    echo "$FECHA_HORA: ERROR --> el script debe ejecutarse con usuario y contraseÃ±a de BD ($DB_INSTANCE)" >> $LOG_FILE
	echo "$FECHA_HORA: ERROR --> Error de parÃ¡metros: el script debe ejecutarse con usuario y contraseÃ±a de BD ($DB_INSTANCE)"
...
if [ "$2" != "" ]; then
    ...
    echo "$FECHA_HORA: ERROR --> el script debe ejecutarse con usuario y contraseÃ±a de BD ($DB_INSTANCE). Solo se ha ingresado usuario" >> $LOG_FILE
	echo "$FECHA_HORA: ERROR --> Error de parÃ¡metros: el script debe ejecutarse con usuario y contraseÃ±a de BD ($DB_INSTANCE). Solo se ha ingresado usuario"
	echo ">>>> Modo de uso: sh $REQ_CAMBIO.sh  [user] ['password']" >> $LOG_FILE
	echo ">>>> Modo de uso: sh $REQ_CAMBIO.sh  [user] ['password']"
	echo ">>>> NOTA: el password debe ingresarse encomillado. Ej: 'password'"
	echo "$FECHA_HORA: FIN NOK. REVISAR LOG --> $LOG_FILE"
...
FECHA_HORA=$(date +%y%m%d.%H%M%S)
echo "$FECHA_HORA: Se validan parÃ¡metros - FIN OK" >> $LOG_FILE
echo "$FECHA_HORA: Se validan parÃ¡metros - FIN OK"
