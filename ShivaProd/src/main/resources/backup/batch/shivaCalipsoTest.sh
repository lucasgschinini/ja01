...
NOMBRE_PROCESO=test-shiva-calipsoWsSecurity
...
then
	...
	java -cp ".:${CLASSPATH}" -Djava.io.tmpdir=${TEMP} -Djavax.net.debug=SSL -Djavax.net.ssl.trustStore=${JAVA_HOME}/jre/lib/security/cacerts \
		-Dentorno=${ENTORNO} -Dconfiguracion=${SHIVA_CONFIG} -DnombreLog=${NOMBRE_PROCESO} \
		ar.com.telecom.shiva.batch.CalipsoWsSecurityTestRunner >>$LOG_ERROR >>$LOG_ARCHIVO 2>>$LOG_ERROR	
else
	# Solo para produccion
	java -cp ".:${CLASSPATH}" -Djavax.net.debug=SSL \
	    -Dentorno=${ENTORNO} -Dconfiguracion=${SHIVA_CONFIG} -DnombreLog=${NOMBRE_PROCESO} \
		ar.com.telecom.shiva.batch.CalipsoWsSecurityTestRunner >>$LOG_ERROR >>$LOG_ARCHIVO 2>>$LOG_ERROR
fi
######################### Resultado ###########################
