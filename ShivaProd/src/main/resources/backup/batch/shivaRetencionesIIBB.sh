...
NOMBRE_PROCESO=batch-shiva-retencionesIIBB
...
#################### Validacion de Fecha ############################
validarArgumentoFechaMesAnio $1
RESULTADO=$?
...
if [ $RESULTADO -lt 1 ]; then
	...
	then
		...
		java -cp ".:${CLASSPATH}" -Djava.io.tmpdir=${TEMP} \
			-Xms1303m -Xmx2048m -XX:MaxPermSize=256m \
			-Dentorno=${ENTORNO} -Dconfiguracion=${SHIVA_CONFIG} -DnombreLog=${NOMBRE_PROCESO} \
	   		ar.com.telecom.shiva.batch.ReporteRetencionesIIBBBatchRunner $1 >>$LOG_ERROR >>$LOG_ARCHIVO 2>>$LOG_ERROR
	else
		# Solo para produccion
		java -cp ".:${CLASSPATH}" -Dentorno=${ENTORNO} -Xms1303m -Xmx2048m -XX:MaxPermSize=256m \
			-Dconfiguracion=${SHIVA_CONFIG} -DnombreLog=${NOMBRE_PROCESO} \
	   		ar.com.telecom.shiva.batch.ReporteRetencionesIIBBBatchRunner $1 >>$LOG_ERROR >>$LOG_ARCHIVO 2>>$LOG_ERROR
	fi
