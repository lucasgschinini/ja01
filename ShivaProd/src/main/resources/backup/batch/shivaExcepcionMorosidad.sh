...
NOMBRE_PROCESO=batch-shiva-excepcionMorosidad
...
then
	...
	if (test "${ENTORNO}" = "desa3")
	then
		PATH=/home/${USER}/jdk1.7.0_25/bin:/usr/bin:${PATH}
	else
...
. ${SHIVA_BATCH_HOME}/shivaFunciones.sh
#################### Validacion de Fecha ############################
...
if [ $RESULTADO -lt 1 ]; then
	...
	then
		...
		java -cp ".:${CLASSPATH}" -Djava.io.tmpdir=${TEMP} \
		 	-Xms1303m -Xmx1303m -XX:MaxPermSize=256m -Dentorno=${ENTORNO} -Dconfiguracion=${SHIVA_CONFIG}  -DnombreLog=${NOMBRE_PROCESO} \
	   		ar.com.telecom.shiva.batch.InterfazExcepcionMorosidadBatchRunner $1 >>$LOG_ERROR >>$LOG_ARCHIVO 2>>$LOG_ERROR
	else
		# Solo para produccion
		java -cp ".:${CLASSPATH}" -Dentorno=${ENTORNO} -Xms1303m -Xmx1303m -XX:MaxPermSize=256m -Dconfiguracion=${SHIVA_CONFIG}  -DnombreLog=${NOMBRE_PROCESO} \
	   		ar.com.telecom.shiva.batch.InterfazExcepcionMorosidadBatchRunner $1 >>$LOG_ERROR >>$LOG_ARCHIVO 2>>$LOG_ERROR
	fi
	######################### Resultado ###########################
	RESULTADO=$?
fi
echo Fecha Hasta ingresada: $1 >>$LOG_ARCHIVO
