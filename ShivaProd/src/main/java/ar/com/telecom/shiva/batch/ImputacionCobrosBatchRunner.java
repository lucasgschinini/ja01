...
public class ImputacionCobrosBatchRunner {

	private ImputacionCobrosBatchRunner(){ 
	}
	...
	public static void main(String[] args) {
		...
		try {
			Traza.auditoria(ImputacionCobrosBatchRunner.class, ControlMemoriaCPU.getInformacionGeneralSistema());
			...
			if (!UnicaInstanciaFileKey.setLock(Constantes.LOCK_KEY_IMPUTACION_BATCH)) {
			...
			for (int i=0; i <= 50; i++) {
				...
				Traza.auditoria(ImputacionCobrosBatchRunner.class, "2.- ===================================================================================");
				procesarTareaImputacionCobros(i);
				
				Traza.auditoria(ImputacionCobrosBatchRunner.class, "FIN ===================================================================================");
				
				try {
					Traza.auditoria(ImputacionCobrosBatchRunner.class, "EN ESPERA..... --> " + ControlMemoriaCPU.getInformacionMemoria());
					
					//Libero la memoria
					System.gc();
					Thread.sleep(10000);
					
					Traza.auditoria(ImputacionCobrosBatchRunner.class, "ARRANCA DE NUEVO..... --> " + ControlMemoriaCPU.getInformacionMemoria());
			...
			Traza.auditoria(ImputacionCobrosBatchRunner.class, 
					"Se ha finalizado el Batch para la imputacion de cobros");
			
			/**Tiempo del proceso batch*/
			double elapsedTime = System.nanoTime() - fechaHoraInicioNanoTime;
			String detalle = "Tiempo de procesamiento batch de imputacion de cobros: " 
						+ ((double) elapsedTime / 1000000000.0);
			Traza.tiempo(ImputacionCobrosBatchRunner.class, "main", detalle);
		}
		System.exit(0);
	}
	...
	private static void procesarTareaMensajesCobros() throws ShivaExcepcion {
		try {
			...
			List<Long> lista = cobroBatchSoporteImputacionMicServicio.listarCobrosPendientesProcesarMIC();
...
//				}
	 ...
	 * @throws ShivaExcepcion 
	 */
	private static void procesarTareaImputacionCobros(int i) throws ShivaExcepcion {
		try {
			...
			cobroImputacionExecutor.ejecutarImputacionesCobros();
			...
			if (i == 50){
				...
				while (!finalizarBatch){
					...
					cantidadHilosVivos = cobroServicio.hayHilosCobrosVivos(TipoImputacionEnum.AUTOMATICA);
					
					if (cantidadHilosVivos > 0){
						Traza.auditoria(ImputacionCobrosBatchRunner.class, "---- Quedan " + cantidadHilosVivos + "hilos ejecutando.");
						Thread.sleep(2000);
						...
						Traza.auditoria(ImputacionCobrosBatchRunner.class, "---- Finalizaron todos los hilos.");
						finalizarBatch = true;
					}
				}
			}
    		...
    		Traza.auditoria(ImputacionCobrosBatchRunner.class, "---- Se ha finalizado exitosamente el proceso de Imputacion de Cobros");
		
		} catch (Throwable e) {
			Traza.advertencia(ImputacionCobrosBatchRunner.class, "---- Se ha finalizado con error en el proceso de Imputacion de Cobros");
			throw new ShivaExcepcion(e);
		}
	}
