...
import ar.com.telecom.shiva.negocio.servicios.IExcelBusquedaValorServicio;
import ar.com.telecom.shiva.presentacion.bean.dto.ValorDto;

/**
 * Clase de Exportacion de la busqueda de Valores.
 * 
 * @author u562163 
 */
public class ExportacionBusquedaValoresImpl implements IExcelBusquedaValorServicio {
	...
	public void generarExportacionBusquedaValores(HttpServletResponse response, List<ValorDto> listaValores) throws NegocioExcepcion {
		...
		try {
			...
			if(Validaciones.isCollectionNotEmpty(listaValores)){
				...
				//Creo la planilla
				HSSFWorkbook workbook = createBook(listaValores);
		    	...
		    	//Nombre del archivo
		    	response.setHeader(CONTENT_DISPOSITION, ATTACHMENT_FILENAME + "Busqueda valores" + xls);
	 ...
	 */
	protected HSSFWorkbook createBook(List<ValorDto> listaValores) throws NegocioExcepcion{
		...
		//Hoja
		HSSFSheet sheet = workbook.createSheet("Busqueda de valores");//Nombre de la hoja

		pintarFondoBlancoHoja(sheet);
		
		int cantidadColumnas = fillTablaValores(workbook, sheet, listaValores);
	...
	private int fillTablaValores(HSSFWorkbook workbook, HSSFSheet sheet, List<ValorDto> listaValores) {
		...
		try {
			debitosFinal = Utilidad.guionesNull(listaValores);
